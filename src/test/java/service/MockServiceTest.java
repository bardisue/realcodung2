...
public class MockServiceTest {
 ...
 //       when(champion.getName()).thenReturn("카이사");
    ...
    @Test
    public void shouldAtLeastTwoTimeRunForChampionArraySize(){
        Champion topChamp = mock(Champion.class);
        topChamp.setName("티모");
        topChamp.setPosition("탑");
        topChamp.setHasSkinCount(3);
        ...
        System.out.println("name :: " + topChamp.getName());
        System.out.println("name :: " + topChamp.getName());
        verify(topChamp, atLeast(3)).getName();
    }
    // 4-2. champion 객체에서 이름을 가져오는 로직이 최소 3번 이하 실행되면 Pass 하는 로직을 작성하세요.
    @Test
    public void shouldAtMostThreeTimeRunForChampionArraySize(){
    ...
    @Test
    public void shouldPassWhenNeverTunChampionSetName(){
 ...
 //       topChamp.setName("티모");
    ...
    @Test
    public void shouldOneTimeRunforChampionGetNameInTime(){

        Champion topChamp = mock(Champion.class);
        topChamp.setName("티모");
        topChamp.setName("르블랑");
        topChamp.setPosition("탑");
        topChamp.setHasSkinCount(3);

        verify(topChamp, timeout(200).atLeastOnce()).setName("르블랑");
    }
    ...
    @Test
    public void 전체챔피언호출시갯수맞는지확인(){
        List<Champion> Champions = new ArrayList<>();

        Champions.add(new Champion("티모", "탑", 1));
        Champions.add(new Champion("다리우스", "정글", 4));
        Champions.add(new Champion("하이머딩거", "원딜", 3));
